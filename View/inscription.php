...
if(isset($_POST['submit']))
{
    if(!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['email']) && !empty($_POST['mdp']) && !empty($_POST['mdp2']) && !empty($_POST['numRue']) && !empty($_POST['nomRue']) && !empty($_POST['nomVille']) && !empty($_POST['cp']) && !empty($_POST['tel'])) //vérifie si le formulaire n'est pas vide et récupère toutes les informations saisies par l'utilisateur dans le formulaire
    {
        ...
        $cp = $_POST['cp'];
        $tel = $_POST['tel'];
        if($mdp == $mdp2){ //vérifie que le mdp saisi est le même que celui de confirmation
            if(strlen($mdp) >= 10){ //vérifie la longueur du mdp
                $mdp = md5($mdp); //crypte le mdp
                $statement = $pdo->prepare("SELECT * FROM user WHERE mail = :email"); //récupère les infos de l'utilisateur qui a créé un compte avec cette adresse mail pour vérifier si elle est déjà utilisée ou non
                $statement->bindValue(':email', $email, PDO::PARAM_STR);
                $statement->execute();
                $result = $statement->fetch(PDO::FETCH_ASSOC);
                if(!$result){
                    $statement = $pdo->prepare("INSERT INTO user (nom, prenom, mail, tel, mdp, numRue, nomRue, nomVille, cp, avatar, reset_token, reset_expires) VALUES (:nom, :prenom, :email, :tel, :mdp, :numRue, :nomRue, :nomVille, :cp, NULL, NULL, NULL)"); // si l'adresse mail n'est pas utilisée, requête qui permet d'insérer les données saisies dans la bdd
                    $statement->execute([
                        ':nom' => $nom,
                        ':prenom' => $prenom,
                        ':email' => $email,
