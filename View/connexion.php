...
if(isset($_POST["submit"])){
    if(!empty($_POST["email"]) && !empty($_POST["mdp"])){ // vérifie que le formulaire n'est pas vide et récupère les informations saisies
        $email = $_POST["email"];
        $mdp = $_POST["mdp"];
        
        $statement = $pdo->prepare("SELECT * FROM user WHERE mail = :email"); // récupère les informations depuis la bdd de l'utilisateur qui a ce mail
        $statement->execute([':email' => $email]);
        $result = $statement->fetch();
        
        if($result && password_verify($mdp, $result['mdp'])){ // vérifie le mot de passe haché
            ...
            <div class="row flex-grow">
                <div class="col-lg-6 d-flex align-items-center justify-content-center">
                    <div class="auth-form-transparent text-left p-3">
                        ...
                        <?php
                        if(isset($_SESSION['inscription'])){ // affiche un message si la session inscription est active, c'est à dire si l'utilisateur est redirigé vers cette page après avoir effectué son inscription
                        ...
                        <span class="input-group-text bg-transparent border-right-0">
                          <i class="mdi mdi-account-outline text-primary"></i>
                            ...
                            <div class="my-3 text-center">
                                <input type="submit" name="submit" class="btn btn-success btn-lg btn-block" value="Connexion">
                            </div>
                          ...
                          <a href="forgot_password.php" class="text-warning mt-5">Mot de passe Oublié</a> <!-- redirection vers la page inscription -->
                          </div>
